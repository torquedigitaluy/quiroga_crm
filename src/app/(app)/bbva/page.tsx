...
export default async function BBVAPage({ searchParams }: { searchParams: Promise<{ mes?: string }> }) {
  ...
  const creditos = await db.creditoBBVA.findMany({
    where: dateFilter ? { fechaFirma: dateFilter } : {},
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-semibold text-foreground">Créditos BBVA</h1>
        ...
        <TableBody>
          {creditos.map((c) => (
            <TableRow key={c.id}>
              ...
              <TableCell>
                <EstadoCreditoSelect id={c.id} estado={c.estado} editable={editable} />
              </TableCell>
            </TableRow>
          ))}
          {creditos.length === 0 && (
